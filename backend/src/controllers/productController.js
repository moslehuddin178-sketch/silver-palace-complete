...
const { generateProductQR, generateProductQRUrl } = require('../utils/qrcode');
...
const createProduct = async (req, res) => {
  ...
  try {
    ...
    const slug = slugify(`${req.body.name}-${skuUpper}`, { lower: true, strict: true });

    const product = await Product.create({ ...req.body, sku: skuUpper, slug, createdBy: req.user._id });
    ...
    res.status(201).json({ success: true, message: 'Product created with QR', data: { ...product.toObject(), prices } });
...
const getProducts = async (req, res) => {
  try {
    const {
      category, status, purity, gender, finish, isFeatured, isNewArrival,
      isBestSeller, availableForWholesale, isLowStock, warehouse,
    ...
    // Wholesaler role sees only wholesale-available products
    ...
    // Exclude qrCode from list (heavy base64)
    ...
    const silverGramPrice = (await require('../models/SilverPrice').getActive());
    ...
    res.status(200).json({ success: true, total, page: Number(page), pages: Math.ceil(total / Number(limit)), data: enriched });
  } catch (err) { res.status(500).json({ success: false, message: err.message }); }
};
...
// ─── SCAN BY SKU OR ID (POS endpoint) ────────────────────────────────────────
const scanProduct = async (req, res) => {
  try {
    ...
    if (!found) return res.status(404).json({ success: false, message: 'Product not found by scan' });
...
const updateProduct = async (req, res) => {
  try {
    ...
    if (!product) return res.status(404).json({ success: false, message: 'Product not found' });
    // Regen QR after update
    const prices = await computePrices(product);
    const { qrBase64, qrString } = await generateProductQR(product, prices);
    product.qrCode = qrBase64; product.qrData = qrString;
    await product.save();
...
const adjustStock = async (req, res) => {
  ...
  try {
    ...
    if (type === 'add')    product.stockQty += qty;
      ...
      if (product.stockQty < qty) return res.status(400).json({ success: false, message: 'Insufficient stock' });
      product.stockQty -= qty;
    } else product.stockQty = qty;
    product.updatedBy = req.user._id;
    await product.save();
    res.status(200).json({ success: true, message: `Stock ${type}: now ${product.stockQty} units`, data: { sku: product.sku, stockQty: product.stockQty, status: product.status } });
  } catch (err) { res.status(500).json({ success: false, message: err.message }); }
};
...
const deleteProduct = async (req, res) => {
  try {
    ...
    if (product.stockQty > 0)
      return res.status(400).json({ success: false, message: 'Cannot delete product with stock. Zero out first.' });
...
const getProductAnalytics = async (req, res) => {
  try {
    const [byCategory, byPurity, lowStock, topSellers, totals] = await Promise.all([
      Product.aggregate([{ $group: { _id: '$category', count: { $sum: 1 }, totalQty: { $sum: '$stockQty' }, totalGrams: { $sum: { $multiply: ['$weightGram', '$stockQty'] } } } }, { $sort: { count: -1 } }]),
      Product.aggregate([{ $group: { _id: '$purity', count: { $sum: 1 }, avgWeight: { $avg: '$weightGram' } } }]),
      Product.find({ isLowStock: true }).select('sku name stockQty minimumStock warehouse').limit(20),
      Product.find().select('sku name totalSold totalRevenue').sort({ totalSold: -1 }).limit(10),
      Product.aggregate([{ $group: { _id: null, totalProducts: { $sum: 1 }, totalStockGrams: { $sum: { $multiply: ['$weightGram', '$stockQty'] } }, totalItems: { $sum: '$stockQty' }, outOfStock: { $sum: { $cond: [{ $eq: ['$status', 'out_of_stock'] }, 1, 0] } }, lowStockCount: { $sum: { $cond: ['$isLowStock', 1, 0] } } } }]),
    ]);
    res.status(200).json({ success: true, data: { overview: totals[0], byCategory, byPurity, lowStock, topSellers } });
  } catch (err) { res.status(500).json({ success: false, message: err.message }); }
};

module.exports = { createProduct, getProducts, getProduct, scanProduct, regenerateQR, updateProduct, adjustStock, deleteProduct, getProductAnalytics };
